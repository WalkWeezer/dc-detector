...
# 🔥 DC-Detector - Скрипт запуска системы детекции огня
...
check_root() {
    if [ "$EUID" -eq 0 ]; then
        print_warning "Не рекомендуется запускать от root"
...
check_camera() {
    ...
    if command -v vcgencmd >/dev/null 2>&1; then
        ...
        if echo "$camera_status" | grep -q "supported=1 detected=1"; then
            ...
            print_warning "Камера не обнаружена через vcgencmd"
        fi
    fi
...
# Основная функция
main() {
    print_header "DC-Detector - Система детекции огня"
