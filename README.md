...
В режиме Raspberry Pi фронтенд автоматически определяет наличие серверной камеры и использует видеопоток с сервера (Pi Camera), а не локальную камеру клиентского ПК. Это позволяет подключаться к системе по локальной сети или Ethernet и видеть поток именно с камеры Raspberry Pi.
